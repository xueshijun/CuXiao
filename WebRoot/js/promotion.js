    ...
    if (result.promotionInfoList.length > 0) {
        ...
        $.each(jsonData, function(i, item) {
            ...
            if (item.promoType == 1) {
                ...
                //限账户限IP
                var limitUserType = item.limitUserType;
                //拼接单品促销信息
                var discountInfo = "";

                if (limitUserType == 4 && discount > 0) {
                    discountInfo += "已优惠￥" + discount;
                } else if (minNum > 1 && maxNum <= 0 && price > 0) {
                    discountInfo += "购买" + minNum + "件及以上：￥" + price;
                } else if (minNum <= 0 && maxNum > 1 && price > 0) {
                    discountInfo += "优惠价：￥" + price;
                } else if (minNum > 1 && maxNum > 1 && price > 0) {
                    discountInfo += "购买" + minNum + "件及以上：￥" + price;
                } else {
                ...
                //新加的
                if(limitUserType == 4 || maxNum > 1) {
                    discountInfo += "，购买超过"+maxNum+"件时不享受该优惠";
                }
            ...
            if (item.promoType == 2) {
                ...
                if (giftInfo != "") {
                    giftTotalInfo = "促销信息：<font color=\"red\">购买" + minNum + "件该商品，可获得以下赠品</font><br/><br/>" + giftTotalInfo;
                    giftTotalInfo += giftInfo + "</div>";
                    $("#liLargess").html(giftTotalInfo);
                    $("#liLargess").show();
                }
 	...
 	$(".btn-subsidies").remove();$("#InitCartUrl").removeClass("btn-appendsmall");
    if (tag == 1 || result.subsidyMoney != null) {
        if (result.subsidyMoney > 0) {
            unitInfor += "<div><font color='red'>参加节能补贴，下单立减￥" + parseFloat(result.subsidyMoney).toFixed(2) + "</font></div>";

            var skuId = $("#name").attr("pshowskuid");
            if ($(".btn-subsidies").length <= 0 && $("#InitCartUrl").length > 0) {
                $('.btns').prepend('<a class="btn-subsidies" href="http://jd2008.360buy.com/purchase/orderinfo_elePow.aspx?pid=' + skuId + '&pcount=' + $("#pamount").val() + '" clstag="shangpin|keycount|product|jieneng">参加节能补贴</a>');
				$("#InitCartUrl").addClass("btn-appendsmall").html("加入购物车");
            }
            ...
            // 修改原有onclick事件
            $('.reduce,.add').removeAttr('onclick').unbind('click').bind('click', function() {
                var input = $('#pamount');
                ...
                if ($(this).hasClass('reduce')) {
                    ...
                    $('.btn-subsidies').attr('href', 'http://jd2008.360buy.com/purchase/orderinfo_elePow.aspx?pid=' + skuId + '&pcount=' + $("#pamount").val());
            ...
            $('#pamount').removeAttr('onkeyup').unbind('keyup').bind('keyup', function() {
                var input = $('#pamount');

                if (parseInt(input.val()) < 1 || parseInt(input.val()) > 999) {
                    alert("请输入正确的数量！");
                    return false;
                }
